...
func (hc *HistoryCreate) SetWord(s string) *HistoryCreate {
	hc.mutation.SetWord(s)
	return hc
}
...
func (hc *HistoryCreate) check() error {
	...
	if _, ok := hc.mutation.Word(); !ok {
		return &ValidationError{Name: "word", err: errors.New(`ent: missing required field "History.word"`)}
	}
	if _, ok := hc.mutation.CreatedAt(); !ok {
...
func (hc *HistoryCreate) createSpec() (*History, *sqlgraph.CreateSpec) {
	...
	if value, ok := hc.mutation.Word(); ok {
