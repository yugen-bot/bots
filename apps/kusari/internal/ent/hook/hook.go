...
// The PlayerSavesFunc type is an adapter to allow the use of ordinary
// function as PlayerSaves mutator.
type PlayerSavesFunc func(context.Context, *ent.PlayerSavesMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PlayerSavesFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PlayerSavesMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PlayerSavesMutation", m)
}
