...
func (hq *HistoryQuery) QueryGame() *GameQuery {
	query := (&GameClient{config: hq.config}).Query()
...
func (hq *HistoryQuery) WithGame(opts ...func(*GameQuery)) *HistoryQuery {
	query := (&GameClient{config: hq.config}).Query()
...
func (hq *HistoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*History, error) {
	...
	if query := hq.withGame; query != nil {
		if err := hq.loadGame(ctx, query, nodes, nil,
			func(n *History, e *Game) { n.Edges.Game = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (hq *HistoryQuery) loadGame(ctx context.Context, query *GameQuery, nodes []*History, init func(*History), assign func(*History, *Game)) error {
...
func (hq *HistoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := hq.ctx.Fields; len(fields) > 0 {
		...
		if hq.withGame != nil {
			_spec.Node.AddColumnOnce(history.FieldGameID)
		}
	}
