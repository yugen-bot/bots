...
func (gq *GameQuery) QueryHistory() *HistoryQuery {
	query := (&HistoryClient{config: gq.config}).Query()
...
func (gq *GameQuery) WithHistory(opts ...func(*HistoryQuery)) *GameQuery {
	query := (&HistoryClient{config: gq.config}).Query()
...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := gq.withHistory; query != nil {
		if err := gq.loadHistory(ctx, query, nodes,
			func(n *Game) { n.Edges.History = []*History{} },
			func(n *Game, e *History) { n.Edges.History = append(n.Edges.History, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GameQuery) loadHistory(ctx context.Context, query *HistoryQuery, nodes []*Game, init func(*Game), assign func(*Game, *History)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(history.FieldGameID)
	}
	query.Where(predicate.History(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(game.HistoryColumn), fks...))
	}))
