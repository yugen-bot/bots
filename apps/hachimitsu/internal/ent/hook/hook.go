...
// The HoneypotFunc type is an adapter to allow the use of ordinary
// function as Honeypot mutator.
type HoneypotFunc func(context.Context, *ent.HoneypotMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f HoneypotFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HoneypotMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HoneypotMutation", m)
}
