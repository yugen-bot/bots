...
func (f GuessFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GuessMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GuessMutation", m)
}

// The PlayerHintsFunc type is an adapter to allow the use of ordinary
// function as PlayerHints mutator.
type PlayerHintsFunc func(context.Context, *ent.PlayerHintsMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PlayerHintsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PlayerHintsMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PlayerHintsMutation", m)
}
