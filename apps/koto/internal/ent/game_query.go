...
func (gq *GameQuery) QueryGuesses() *GuessQuery {
	query := (&GuessClient{config: gq.config}).Query()
...
func (gq *GameQuery) Clone() *GameQuery {
	...
	return &GameQuery{
		...
		withGuesses: gq.withGuesses.Clone(),
...
func (gq *GameQuery) WithGuesses(opts ...func(*GuessQuery)) *GameQuery {
	query := (&GuessClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGuesses = query
	return gq
}
...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := gq.withGuesses; query != nil {
		if err := gq.loadGuesses(ctx, query, nodes,
			func(n *Game) { n.Edges.Guesses = []*Guess{} },
			func(n *Game, e *Guess) { n.Edges.Guesses = append(n.Edges.Guesses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GameQuery) loadGuesses(ctx context.Context, query *GuessQuery, nodes []*Game, init func(*Game), assign func(*Game, *Guess)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(guess.FieldGameID)
	}
	query.Where(predicate.Guess(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(game.GuessesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "gameID" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
