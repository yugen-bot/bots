...
type PlayerStats struct {
	...
	// Participated holds the value of the "participated" field.
...
func (*PlayerStats) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case playerstats.FieldID, playerstats.FieldPoints, playerstats.FieldParticipated, playerstats.FieldWins:
...
func (ps *PlayerStats) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case playerstats.FieldParticipated:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field participated", values[i])
			} else if value.Valid {
				ps.Participated = int(value.Int64)
			}
		case playerstats.FieldWins:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field wins", values[i])
			} else if value.Valid {
				ps.Wins = int(value.Int64)
			}
...
func (ps *PlayerStats) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("participated=")
	builder.WriteString(fmt.Sprintf("%v", ps.Participated))
	builder.WriteString(", ")
	builder.WriteString("wins=")
	builder.WriteString(fmt.Sprintf("%v", ps.Wins))
	builder.WriteString(", ")
