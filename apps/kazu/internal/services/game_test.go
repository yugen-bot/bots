...
func parseNumberPure(content string, isBot bool, math bool) (int, error) {
	...
	if !math {
		...
		if err != nil {
			return i, fmt.Errorf("parse number: %w", err)
		}
...
func parseNumberTestCases() []parseNumberCase {
	return []parseNumberCase{
		{
...
func runParseNumberCase(t *testing.T, tc parseNumberCase) {
	t.Helper()
	...
	case !tc.wantErrSome && gotErr != nil:
...
func TestParseNumberPure(t *testing.T) {
	for _, tc := range parseNumberTestCases() {
		t.Run(tc.name, func(t *testing.T) {
			runParseNumberCase(t, tc)
		})
	}
}
